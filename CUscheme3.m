function [y1,y2,dt] = CUscheme3(U1,U2,dx,q,dv,v,xT)
    CFL = 0.5;
    n = length(U1);
    ...
    cm = dv(um1,um2,um,1,1).*um1.*v(um,2)+...
        ...
        um1.*um2.*(dv(um1,um2,um,2,1).*dv(um1,um2,um,1,2)-dv(um1,um2,um,1,1).*dv(um1,um2,um,2,2));
    ...
    cp = dv(up1,up2,up,1,1).*up1.*v(up,2)+...
        ...
        up1.*up2.*(dv(up1,up2,up,2,1).*dv(up1,up2,up,1,2)-dv(up1,up2,up,1,1).*dv(up1,up2,up,2,2));

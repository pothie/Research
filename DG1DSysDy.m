function[u,UT]=DG1DSysDy(x,u,h,m,N,CFL,FinalTime,q,v,dv,xT)
...
uT(1,:) = xT(um);
uT(2,:) = xT(up);
b = dv(uc1,uc2,uT,1,1).*uc1+v(uT,1)+dv(uc1,uc2,uT,2,2).*uc2+v(uT,2);
c = dv(uc1,uc2,uT,1,1).*uc1.*v(uT,2)+...
    dv(uc1,uc2,uT,2,2).*uc2.*v(uT,1)+v(uT,1).*v(uT,2)-...
    uc1.*uc2.*(dv(uc1,uc2,uT,2,1).*dv(uc1,uc2,uT,1,2)-dv(uT,1,1).*dv(uT,2,2));
...
% change the window according to x-grid
...
UT(:,tstep) = [xT(u(1:2:3,:)) xT(u(2:2:4,end))];
end
return
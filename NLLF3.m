...
% Uend1: U(end+1,t)
% U1: U(-1,t)
function [U,U1,U2,tgrid] = NLLF3(x,T,ux0,v,dv,q,ut0,pce)
    pt = [0.25 10;0.5 50;1 50;1.25 10];
    dis = [0.8 0.2];
    if T == 0
        U = ut0;
    elseif x == 0
        U1(:,1) = dis(1)*Up(0:1.5e-3:T,pt);
        U2(:,1) = dis(2)*Up(0:1.5e-3:T,pt);
        U = pce*[U1 U2]';
    else
    CFL = 0.9;
    dx = x(2)-x(1);
    ...
    while T-tgrid(tstep) > 0
        ...
        U1(end,tstep+1) = dis(1)*Uend1;
        U2(end,tstep+1) = dis(2)*Uend1/pce(2);
        
        Un1 = dis.*Up(tpass,pt)';
        U1(1,tstep+1) = Un1(:,1); % Given U(0,t)
        U2(1,tstep+1) = Un1(:,2)/pce(2);
